...
| コマンド | 説明 |
|---------|------|
| `agentform plan` | YAML 定義と現在の state を比較し、差分を表示 |
| `agentform apply` | plan の内容を実行して API に反映、state を更新 |
...
| サブコマンド | 説明 |
|------------|------|
| `agentform state refresh` | API から最新状態を取得して state を再構築 |

## グローバルオプション (Phase 2)

| オプション | 説明 |
|-----------|------|
| `--config <path>` | 設定ファイルパス (デフォルト: `./agentform.yaml`) |
| `--state <path>` | State ファイルパス (デフォルト: `./agentform.state.json`) |
