# agentform - Initial Design

## Overview

agentform は Claude Managed Agent のリソース（Agent, Skill, Environment）を宣言的に管理する CLI ツール。
Terraform のような plan/apply のワークフローで、YAML ファイルに定義されたリソースを API と同期する。

## Positioning

```
agentform (インフラ管理)          プロダクトコード (アプリケーション層)
┌─────────────────────────┐      ┌──────────────────────────────────┐
...
agentform 自体は session を作成しない。リソースの存在とその設定を管理するだけ。
...
- Language: TypeScript
- Config format: YAML
- State: ローカル JSON ファイル (`agentform.state.json`)
- API client: `@anthropic-ai/sdk`

## CLI Commands

```bash
agentform plan      # YAML 定義と現在の state を比較し、差分を表示（YAML から消えたリソースは destroy として表示）
agentform apply     # plan の内容を実行して API に反映、state を更新（create / update / destroy 全て含む）
agentform destroy   # state にある全リソースを削除
agentform state     # 現在の state を表示
```
...
- update API は存在しない。`display_title` の変更は delete → create で対応。
...
- agentform 固有フィールド（`directory`, ファイルパス参照, `${...}` 式）を統一的に扱う
...
- agentform 固有の拡張（ファイル参照、参照式）を自然に組み込める
...
- `${file('./path')}`: ファイル内容を文字列として注入。パスは `agentform.yaml` の位置を基準に相対解決。
- `directory` フィールド (Skill): ディレクトリパスを文字列として指定。`agentform.yaml` の位置を基準に相対解決し、中のファイルをアップロード。
...
Phase 2:
- [ ] `destroy` のリソース指定オプション（例: `agentform destroy agent.support-bot`）
- [ ] `state refresh` コマンド
- [ ] 並列実行（同階層リソース）
- [ ] dry-run モード
